...
class BankUi:
    ...
    def video_check(self):
        ...
        while True:
            ...
            for i in range(0, detections.shape[2]):
                ...
                if confidence > 0.5:
                    ...
                    #Decision boundary
                    name_counts = Counter(real_user_list)
                    if (name == 'unknown') or name_counts[name] < 60:
        ...
        try:
            most_common_user = Counter(real_user_list).most_common(1)
            if most_common_user and most_common_user[0][0] == 'unknown':
                # Handle the case when the most common user is 'unknown'
                if self.countter > 0:  # Only decrement if countter is above 0
                    ...
                    self.begin_page()
                    self.countter = 1  # Reset counter here if needed
            else:
                # Check if both 33439 and 12146 are in the real_user_list
                if '33439' in real_user_list and '12146' in real_user_list or '84557' in real_user_list or '49556' in real_user_list:
                    if self.countter > 0:  # Only decrement if countter is above 0
                        messagebox._show("Verification Info!", "Face Id match failed! You have {} trials left".format(self.countter))
                        self.countter -= 1
                        self.video_check()
                    ...
                    # If the most common user is not 'unknown'
                    self.real_user = int(most_common_user[0][0])
                    messagebox._show("Verification Info!", "Face Id match!")
                    subprocess.Popen(['python', 'keyboard.py'])
                    self.password_verification()

        except IndexError:
            # This block will only execute if there's an issue retrieving the most common user
            if self.countter > 0:  # Only decrement if countter is above 0
                messagebox._show("Verification Info!", "Face Id match failed! You have {} trials left".format(self.countter))
                self.countter -= 1
                self.video_check()
